# 실행 중인 도커 컴포즈 확인
EXIST_A=$(docker-compose -p infludeo-a -f docker-compose.a.yml ps | grep infludeo-a)
...
echo "################ infludeo-${START_CONTAINER} up ####################"
docker-compose -p infludeo-${START_CONTAINER} -f docker-compose.${START_CONTAINER}.yml up -d --build || exit 1
...
# 기존에 실행 중 docker-compose 종료.
echo "##################### infludeo-${TERMINATE_CONTAINER} container down ##################"
docker-compose -p infludeo-${TERMINATE_CONTAINER} -f docker-compose.${TERMINATE_CONTAINER}.yml down
